<?php
$this->fileLayout = "Frontend/Views/Layout_trangtrong.php";
?>
<div class="heading-ctn">
    <div class="heading">
        <h1>
            ...
            ?>
            <span>(<?php $conn = Connection::getInstance();
            $query = $conn->prepare("select count(*) as total from tbl_product where category_id=:id");
            $query->execute(array("id" => $id));
            $category_count = $query->fetch();
            echo isset($category_count->total) ? $category_count->total : "";?> sản phẩm)</span></h1>
    </div>
        ...
        <ul class="pagination">
            <?php for ($i = 1; $i <= $numPage; $i++) : ?>
                <li class="page-item active">
                    <a class="page-link" href="index.php?controller=ProductCategory&id=<?php echo  $id; ?>&p=<?php echo $i; ?>"><?php echo $i; ?></a>
                </li>
            <?php endfor; ?>
        </ul>
